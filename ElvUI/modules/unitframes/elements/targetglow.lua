...
function UF:Configure_TargetGlow(frame)
	...
	if(frame.USE_POWERBAR_OFFSET) then
		if(frame.ORIENTATION == "RIGHT") then
			targetHealthGlow:Point("TOPLEFT", frame.Health.backdrop, "TOPLEFT", -frame.SHADOW_SPACING - frame.SPACING - frame.HAPPINESS_WIDTH, frame.SHADOW_SPACING + frame.SPACING + (frame.USE_CLASSBAR and (frame.USE_MINI_CLASSBAR and 0 or frame.CLASSBAR_HEIGHT) or 0))
			...
			targetHealthGlow:Point("BOTTOMRIGHT", frame.Health.backdrop, "BOTTOMRIGHT", frame.SHADOW_SPACING + frame.SPACING + frame.HAPPINESS_WIDTH, -frame.SHADOW_SPACING - frame.SPACING)
		end
